...
pub mod truesight_protocol {
    ...
    pub fn create_prediction(
        ...
        // When there are not enough TSD coins in wallet
            ...
            && ctx.accounts.has_enough_funds(bid_amount)
        {
            ...
            prediction_record.expiry_date =
                (holdout_period_sec as i64) + Clock::get().unwrap().unix_timestamp;

            prediction_record.bidder_token_wallet_key =
                ctx.accounts.user_token_wallet.key().to_string();
